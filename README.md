...
- **설정 가능 항목**: 다운로드 폴더, 수락 확인 크기, 자동 실행 여부, 내 장치 표시 이름, 통신 포트.
...
수신을 위해 TCP 포트(기본 **45671**)가 열려 있어야 합니다. 첫 실행 시 Windows 방화벽 허용 창이 뜨면 **개인 네트워크**에 대해 허용해 주세요. 모든 컴퓨터가 같은 포트를 사용해야 합니다.

## 사용 방법

1. **컴퓨터 등록** — 트레이의 IntraDrop 아이콘을 더블클릭해 컴퓨터 목록을 열고, `추가` 버튼으로 상대방의 IP와 별명을 등록합니다.
2. **파일 보내기** — 목록에서 컴퓨터를 더블클릭(또는 선택 후 `보내기 창`)하면 작은 드롭 창이 열립니다. 파일/폴더를 끌어다 놓으면 전송됩니다.
3. **파일 받기** — 받은 파일은 설정된 다운로드 폴더(기본: `다운로드\IntraDrop`)에 저장되고 트레이 풍선 알림이 뜹니다. 같은 이름의 파일이 이미 있으면 `이름 (2).확장자` 형식으로 저장됩니다.
4. **설정** — 트레이 아이콘 우클릭 → `설정`에서 다운로드 폴더, 수락 확인 크기(256MB/512MB/1GB), 자동 실행, 포트를 바꿀 수 있습니다.
...
- 보내는 쪽이 접속해 매직 바이트(`IDRP1`)와 JSON 헤더(보낸이, 파일 목록, 총 크기)를 전송합니다.
- 받는 쪽은 크기가 임계값 미만이면 자동 수락, 이상이면 사용자에게 수락 여부를 묻습니다.
- 수락되면 파일 데이터가 순서대로 스트리밍되고, 저장 완료 응답 후 연결이 종료됩니다.
- 수신 경로는 항상 다운로드 폴더 하위로 제한되며(경로 탈출 차단), 전송 중 파일은 `.part`로 저장 후 완료 시 이름이 바뀝니다.

## 보안 참고

- 인트라넷(신뢰된 사설망) 사용을 전제로 하며, 전송 구간 암호화나 사용자 인증은 포함하지 않습니다. 공용 네트워크에서는 사용하지 마세요.
